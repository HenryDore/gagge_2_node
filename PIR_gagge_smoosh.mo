model PIR_gagge_smoosh
  ...
  Real edif "heat loss due to diffusion of water vapour through the skin";
  Real ereq "?";
  Real ecomf "?";
  ...
  //variables for TSENS
  Real wettedness "skin wettedness #2 from ASHRAE";
  Real t_b_c "lower limit for zone of evaporative regulation, °C";
  Real t_b_h "upper limit for zone of evaporative regulation, °C";
...
//calculate PMV/PPD/TSESNS
  ...
  ereq = m - w - eres - cres - dry;
  ecomf = max(0, (0.42*(m - w - 58.2)));
  PMV_gagge = (0.303*exp(-0.036*m) + .028)*(ereq - ecomf - edif);
  wettedness = ersw/emax;
  t_b_c = (0.194/58.15)*(m - w) + 36.301;
  t_b_h = (0.347/58.15)*(m - w) + 36.669;
  if tb < t_b_c then
...
//year
  annotation(
    ...
    __OpenModelica_simulationFlags(lv = "LOG_STATS", r = "controlled.mat", s = "dassl", variableFilter = ".*"),
    experiment(StartTime = 0, StopTime = 3.1536e+07, Tolerance = 1e-05, Interval = 600));
